...
export default class PYWIDEVINE_DEVICE {
    ...
    static parse(buf: Buffer, offset = 0) {
        ...
        if (constant !== 'WVD') throw new Error(`Invalid signature, not a WVD file: ${constant}`)
        offset += 3
        ...
        offset += 1
        if (security_level > 3 || security_level < 1) throw new Error(`Invalid security_level, not a WVD file: security_level ${security_level}`)
        ...
        offset += 2
        // Private Key
        const private_key = buf.subarray(offset, offset + private_key_len) as Buffer<ArrayBuffer>
        offset += private_key_len
        // Client ID Blob Length
        const client_id_len = buf.readInt16BE(offset)
        offset += 2
        // Client ID Blob
        const client_id = buf.subarray(offset, offset + client_id_len) as Buffer<ArrayBuffer>
